...
class TwoStreamControlLDM(LatentDiffusion):
    # Friedrich: TODO: Log images amount as hyperparameters
    def __init__(self, control_stage_config, control_key, sd_locked=True, sync_path=None, synch_control=True,
    ...
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        ...
        if self.control_mode == 'midas':
            ...
            for inp in batch['image']:
                new = (inp / 2. + 1) * 255
                control, _ = self.hint_model(np.array(new.cpu()))
                controls.append(tt.ToTensor()(control[..., None].repeat(3, 2)))
            control = torch.stack(controls).to('cuda')
    ...
    def apply_model(self, x_noisy, t, cond, precomputed_hint=False, *args, **kwargs):
        ...
        cond_hint = torch.cat(cond['c_concat'], 1) if self.control_mode != 'multi_control' else cond['c_concat'][0]
    ...
    @torch.no_grad()
    def log_images(self, batch, N=8, n_row=2, sample=False, ddim_steps=50, ddim_eta=0.0, return_keys=None,
                   quantize_denoised=True, inpaint=True, plot_denoise_rows=False, plot_progressive_rows=True,
    ...
    def load_control_ckpt(self, ckpt_path_ctr):
        ...
        self.control_model.load_state_dict(ckpt['state_dict'])
