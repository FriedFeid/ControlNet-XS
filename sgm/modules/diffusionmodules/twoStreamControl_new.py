...
class TwoStreamControlNet(nn.Module):
    ...
    def scale_controls(self, scale=1, sequence='geometric', start=1, end=1, custom_scales=None):
        assert sequence in ('geometric', 'exponential', 'equal', 'linear', 'custom'), '<sequence> has to be either "exponential", "equal", "geometric", "custom" or "linear"'
        if sequence == 'custom':
            assert custom_scales is not None, 'No custom scales provided'
        if sequence == 'exponential':
            self.control_scale = self.control_scale * scale
        ...
        if custom_scales:
            custom_list = iter(custom_scales)
        for block in possible_blocks:
            if sequence == 'geometric':
                scale_list = [1 / (scale * n) for n in range(1, len(block) + 1)]
            elif sequence == 'exponential':
                scale_list = [scale ** n for n in range(1, len(block) + 1)]
            elif sequence == 'equal':
                scale_list = [scale for _ in range(1, len(block) + 1)]
            elif sequence == 'linear':
                scale_list = [scale * n for n in np.linspace(start, end, len(block))]
            elif sequence == 'custom':
                scale_list = next(custom_list)
            for subblock, factor in zip(block, scale_list):
                # if isinstance(subblock, PseudoModule):
                if "PseudoModule" in str(type(subblock)):
                    continue
                for conv in subblock:
                    if "PseudoModule" in str(type(conv)):
                        continue
    ...
    def forward_(self, x, hint, timesteps, context, base_model=None, y=None, precomputed_hint=False, no_control=False, compute_hint=False, **kwargs):
        ...
        if compute_hint:
            ...
            for inp in hint:
                ...
                # hint_processed, _ = self.hint_model(np.array(new.cpu().permute(1, 2, 0)))
                hints.append(tt.ToTensor()(hint_processed[..., None].repeat(3, 2)))
            hint_processed = torch.stack(hints).to(x.device)
            # hint_processed = rearrange(hint_processed, 'b h w c -> b c h w')
        ...
        if self.two_stream_mode == 'cross':
            ...
            for module_base, module_ctr in zip(base_model.input_blocks, self.control_model.input_blocks):
                ...
                # if not isinstance(module_ctr, PseudoModule):
                ...
                if not self.debugger['skip_enc_infusion']:
                    if self.guiding in ('encoder_double', 'full') and not isinstance(module_ctr[0], Downsample):
                        if self.infusion2base == 'add':
                            ...
                            h_base = h_base + next_module(h_ctr, emb) * next(scales)
                        elif self.infusion2base == 'cat':
                            raise NotImplementedError()
...
class SD15ResBlock(TimestepBlock):
    """
    A residual block that can optionally change the number of channels.
    :param channels: the number of input channels.
    :param emb_channels: the number of timestep embedding channels.
